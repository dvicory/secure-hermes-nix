...
The package was extracted while replacing the V3 Gondolin broker. V3 is a behavioral baseline, not the target product model. The HTTP broker intentionally does **not** preserve V3's framed Unix-socket transport, request envelopes, or module layout. It preserves environment generations, worklanes, bounded exec/file operations, local policy checks, and now composes broker-owned runtime grants at the enforcement edge. Mutable grant state does not live in this pure package.
...
| No grant, approval, or capability reference in `PolicyAction` | The broker PEP owns prepared capabilities and transactional runtime grants outside the pure kernel | References to product-owned grants/approvals with generation, expiry, scope, and single-use/idempotency state | The local broker now passes immutable grant snapshots to Gondolin hooks and consumes once-use transactionally; migrate ownership to Agent X/PostgreSQL without moving SQL or mutable state into the kernel |
| Full immutable policy digest lives in the broker config/registry, not the pure kernel `PolicyDocument` | The broker must fence persisted environments and grants when rendered policy content changes | One authorization snapshot bound to policy, runtime, adapter, lease, and product generations | Define an `AuthorizationSnapshot` contract and include its full digests/generations in decisions and audit records |
...
| Obligation vocabulary exists, but each evaluator advertises only its implemented handlers | Runtime network grants are composed by the broker PEP rather than represented as a model-selected obligation; credentials, audit delivery, redaction, and publication remain absent | Each PEP advertises and proves the obligations it enforces | Implement obligation handlers and tests one PEP at a time; unsupported obligations continue to deny |
| Limits and obligations are returned, not enforced by the kernel | Keeps the kernel pure and reusable | Local PEP atomically enforces authority, consumption, fencing, result publication, and audit | The broker now enforces authority binding and grant consumption; Agent X still needs product-grade publication, audit correlation, and reconciliation |
