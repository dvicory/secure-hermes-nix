...
Protocol support is not permission. The rendered profile policy is the upper bound accepted by the broker. For `hermes-qa`, Nix renders only `once` and `task`. The sandbox plugin schema accepts only those two requested scopes, passes `allow_permanent = false` to the generic approval API, and accepts only `once` and `session` results. The gateway passes that restriction to button adapters which explicitly support restricted choices; Telegram therefore retains buttons for once, session, and deny while omitting `always`. Legacy adapters fall back to an equivalent restricted text prompt until they implement the contract. Forged or stale `always` results fail closed rather than being downgraded. Existing non-sandbox Hermes approval callers retain their default permanent-choice behavior.

The generic Hermes approval core owns the attenuated effective choice and returned choice metadata, including `allow_permanent` attenuation and cache handling. The sandbox plugin owns the mapping from a returned `session` choice to the broker's requested `task` scope. Gateway, CLI, and platform adapters are rendering surfaces only: they MUST render choices permitted by that core call and MUST fail closed on stale or forged `always`. Native v2026.7.20 prompt support for `allow_permanent` is surface plumbing, not the complete local contract.
