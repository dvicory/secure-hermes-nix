...
- [ ] 5.3 Deploy the shared release through the normal `hvn-hyp1` QA canary and production-promotion pipeline while keeping dynamic authority QA-only, run the Hermes-agent prompt/scenario against default, dynamically approved public/private, revoked, and denied paths, record observed compatibility without a production secure-terminal cutover, and commit the verified final configuration.
