...
A worker lane MUST be able to declare its runtime, optional profile, model, reasoning effort, role/SOUL, model-visible tools and skills, durable-memory mode, workspace contract, broker policy worklane, approvals, execution limits, and concurrency. Immutable operator and security policy MUST remain authoritative over lane prompt or tool configuration.

#### Scenario: Hermes lane resolution
- **WHEN** trusted dispatch resolves a Hermes lane
- **THEN** it SHALL produce one worker configuration containing the lane's declared agent, memory, workspace, policy, approval, and execution settings

#### Scenario: Lane prompt attempts to broaden authority
- **WHEN** a lane SOUL or task body asks for a tool, workspace, credential, network grant, or policy capability outside the resolved backend ceiling
...
### Requirement: Nix-authoritative logical Project catalogue
Managed logical Projects MUST have instance-wide stable identity, trusted source configuration, and one per-lane access map declared by Nix. Runtime workflow state MUST NOT add a managed source identity or broaden Project lane access beyond the active Nix catalogue.
